...
class TaskItem(
    ...
    @ColumnInfo(name = "completedDataString") var completedDataString: String?,
    @PrimaryKey(autoGenerate = true) var id: Int = 0
)

{
    fun completedData(): LocalDate? = if (completedDataString == null) null
        else LocalDate.parse(completedDataString, dateFormatter)
    fun dueTime(): LocalTime? = if (dueTimeString == null) null
        else LocalTime.parse(dueTimeString, timeFormatter)

    fun isCompleted() = completedData() != null
